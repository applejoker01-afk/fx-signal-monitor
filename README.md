...
### 2026-06-08 ダマシ低減強化（L3.1）
- `compute_ta_score` に以下の新フィルタを追加
  - トレンド強度フィルタ（DMA乖離率）
  - ATRボラティリティ品質フィルタ
  - 直近20期間ブレイクアウト確認
- `evaluate_full` の星評価閾値を厳しめに調整（★5要求値：TA≥78 / FA≥68）
- 目的：勢いのないレンジ相場での誤シグナル（ダマシ）を大幅に抑制

### 2026-06-08 地政学リスクモジュール追加
- `modules/geopolitical_risk.py` を新規作成
- 新興国通貨（MXN, ZAR, TRY, INR など）に対してニュースベースのリスク評価を実施
- 高リスク検出時は星評価を強制的に1〜2ランク下げ
- `evaluate_full()` に自動適用

### 2026-06-08 地政学リスク日記ログ機能追加
- 評価時に使用したニュースタイトル・リスクカテゴリ・加点内訳を `docs/geopolitical_risk_diary.jsonl` に自動記録
- 後から「どのニュースでなぜ星が下がったか」を振り返り可能
- 運用しながらニュース精度を改善するための学習データとして活用

**Obsidian対応**:
- 毎日 `docs/geopolitical_risk/YYYY-MM-DD.md` が自動生成される
- Obsidian Vaultにコピーすればそのまま日記として使える

### config.yamlでの設定例

```yaml
geopolitical:
  obsidian_vault_path: "/Users/xxx/Obsidian/Vault"   # ここにVaultのルートパスを指定
```
