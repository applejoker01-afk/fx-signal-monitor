...
やること:
  ...
  2. まだ発注していないもの・期限切れでないものを一覧表示
  3. 1件ずつユーザーに承認(y/n/s=スキップ)を確認
  4. 承認したものだけ、実際の推奨ロットを計算してMT5へ指値注文を送信
  5. 送信結果（成功/失敗・チケット番号）を data/mt5_sent_orders.json に記録し、
     次回実行時に同じ注文を二重送信しないようにする
...
def main() -> int:
    ...
    for pair, order in targets.items():
        ...
        if not sizing.get("tradable"):
            print(f"  ⚠ サイジング不可: {sizing.get('note')}")
            print("  → このシグナルはスキップします（yを入力しても発注しません）")

        choice = input("  この指値を発注しますか？ [y]es / [n]o / [s]kip: ").strip().lower()
        if choice != "y":
            print("  スキップしました。")
            continue

        if not sizing.get("tradable"):
            print("  サイジング不可のため発注しません。")
            continue

        units = sizing["units"]
        lots, note = mt5_bridge.units_to_lots(pair, units)
        if lots <= 0:
            print(f"  ⚠ ロット変換失敗: {note}")
            continue

        print(f"  推奨: {units}{PAIR_API[pair][0]} → {lots}ロットで発注します...")
